...
const DNSAutoDiscovery = require('./dnsautodiscovery');
const Timer = require('./timer.js');
const ObservableMemcached = require('./observable-memcached');
const ObservableInMemory = require('./observable-inmemory');
...
function createAutoDiscovery(options) {
    logger.debug('createAutoDiscovery', {options}); //temp
...
function CampbellCache(options) {
    ...
    opts.in_memory = getOrDefault(options.in_memory,false);

    logger.debug({opts}); //temp
    ...
    observable_memcached_opts.in_memory = opts.in_memory;
    ...
    if(!opts.in_memory) {
        if(opts.autodiscovery) {
            ...
            if(opts.hosts.length==0) {
                logger.debug("No memcached endpoints defined.  No Op Client is going to be defined!");

                // Sort out the no operation client
                throw new Error("No memcached endpoints defined.  No Op Client is going to be defined!");
            }
        }
    }
...
CampbellCache.prototype._observableMemcacheFactory = function (hosts,options) {
    if (options.in_memory) {
        return new ObservableInMemory(true,null,options);
    } else {
